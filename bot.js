...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['Helios System','#Helios'];
    var i = -1;
    var j = 0;
...
${prefix}embed = يكرر الي تقولو بشكل حلو بالانقليزي فقط
${prefix}sug = ارسال اقتراح ل صاحب البوت
**
  `
,`
        ***__Administrative Orders__***
**
${prefix}move @user = لسحب الشخص الى رومك
...
var codes = [""]
message.channel.send(":postbox: **شف خاصك ي حلو**")
guild.fetchInvites()
.then(invites => {
...
if (message.content.startsWith(prefix + 'tag')) {
    let args = message.content.split(" ").slice(1);
if(!args[0]) return message.reply('اكتب النص الذي تريده');  
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/GZdsPrr**")
...
client.users.get("327906017092304896").send(
    "\n" + "`" + "● Server :" + "`" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "`" + "● Sender :" + "`" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
message.channel.send("``لا تستطيع سحب "+ message.mentions.members.first() +" `يجب ان يكون هذه العضو في روم صوتي`")
 ...
 message.channel.send("**``ادخل اي روم عشان تقدر تسحب``**")
}
} else {
message.react("?")
 }}});
...
client.on('message', message => {
  ...
  .kickable) return message.reply("**مقدر اعطيه كيك ذا رتبته فوق رتبتي**");
...
client.on("message", async message => {
    ...
    message.channel.send('**هذا الامر لصاحب السيرفر فقط**' );
      return;
    }
    const array = [];
...
client.on('message', async message =>{
    ...
    if(message.content.startsWith(prefix + 'vc')){
        let codes = ""
        var ecodes = new Discord.RichEmbed()
        message.member.voiceChannel.members.forEach(member =>{
            ecodes
            .setTitle('الأشخاص في رومك الصوتي')
            .setDescription(codes += member + '\n')
            .setTimestamp()
        })
        message.channel.send(ecodes)
    }
})
